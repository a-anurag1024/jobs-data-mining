...
class Processor(ABC):
    def __init__(self, config: ProcessorConfig):
        ...
        self.langchain = LangChainPipeline(self.config.langchain_config)
    ...
    def retrieve_info_from_results(self, results) -> dict:
        ...
        Note: Raise error if the output schema is not as expected
        """
        pass
    ...
    def process(self, input_data: dict) -> dict:
        """
        Function which processes one of the object (input data) in the self.objects list
        
        Returns:
        - output: Dict: The output of the processing function
        """
        results = self.langchain.run(input_data)
        
        return self.retrieve_info_from_results(results)
